...
router.get('/rooms', authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
  try {
...
// Create support room
router.post('/rooms', authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
  try {
    ...
    const result = await query(
      `INSERT INTO support_rooms (name, description, max_members, admin_owner_id)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [name, description, max_members || 10, req.userId]
    );
...
// Delete support room
router.delete('/rooms/:roomId', authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
  try {
    const { roomId } = req.params;

    await query('DELETE FROM support_rooms WHERE id = $1', [roomId]);
...
router.post('/login', async (req, res) => {
  try {
    ...
    const result = await query(
      `SELECT id, full_name, email, matricula, is_active
       FROM support_users
