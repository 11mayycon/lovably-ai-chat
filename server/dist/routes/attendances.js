...
router.post('/list', auth_1.authenticateToken, auth_1.requireSupport, async (req, res) => {
    try {
        ...
        const roomsResult = await (0, database_1.query)('SELECT id FROM support_rooms WHERE support_user_id = $1', [support_user_id]);
        if (roomsResult.rows.length === 0) {
            return res.json({ attendances: [] });
        }
        const roomIds = roomsResult.rows.map(r => r.id);
        // Get attendances from those rooms
        const attendancesResult = await (0, database_1.query)(`SELECT a.*, wc.instance_name, wc.phone_number
       FROM attendances a
       LEFT JOIN whatsapp_connections wc ON a.whatsapp_connection_id = wc.id
       WHERE a.room_id = ANY($1) 
       AND a.status IN ('active', 'waiting')
       ORDER BY a.updated_at DESC`, [roomIds]);
...
router.get('/:attendanceId/messages', auth_1.authenticateToken, async (req, res) => {
    try {
        const { attendanceId } = req.params;
        const result = await (0, database_1.query)(`SELECT * FROM messages 
       WHERE attendance_id = $1 
       ORDER BY created_at ASC`, [attendanceId]);
...
// Start bot chat
router.post('/start-bot-chat', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const existingResult = await (0, database_1.query)(`SELECT * FROM attendances 
       WHERE room_id = $1 
       AND agent_id = $2 
       AND assigned_to = 'ai' 
       AND status = 'active'
       LIMIT 1`, [room_id, support_user_id]);
        if (existingResult.rows.length > 0) {
            return res.json({ attendance: existingResult.rows[0] });
        }
        // Create new bot chat attendance
        const newResult = await (0, database_1.query)(`INSERT INTO attendances 
       (room_id, agent_id, client_name, client_phone, assigned_to, status) 
       VALUES ($1, $2, $3, $4, 'ai', 'active') 
       RETURNING *`, [room_id, support_user_id, 'AI Assistant', 'bot']);
